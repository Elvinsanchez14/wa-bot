...
/**
 * Intenta encontrar una respuesta en el banco de frases fijo.
 * Devuelve la frase si hay match, o null si no se reconoció nada
 * (para que quien llame decida qué hacer -- ej. intentar con IA antes del fallback).
 */
function intentarRespuestaFija(texto, remitente = 'desconocido') {
  ...
  return null; // no hubo match -- quien llama decide el siguiente paso
}

function respuestaFallback(remitente = 'desconocido') {
  return elegirSinRepetir(FALLBACK, `${remitente}|fallback`);
}

// Se mantiene por compatibilidad: comportamiento antiguo (banco de frases + fallback
// estático, sin IA), por si algún módulo todavía la usa directamente.
function generarRespuesta(texto, remitente = 'desconocido') {
  const fija = intentarRespuestaFija(texto, remitente);
  return fija !== null ? fija : respuestaFallback(remitente);
}

module.exports = { generarRespuesta, intentarRespuestaFija, respuestaFallback };
