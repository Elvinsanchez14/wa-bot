...
/**
 * Usuarios que entraron al grupo DENTRO de los últimos N minutos (sin importar
 * si ya hablaron o no). Se usa para !antiraid: expulsar en bloque a todos los
 * que entraron recientemente, ante una posible invasión/raid coordinada.
 */
function usuariosEntradaReciente(grupo, minutos) {
  const usuarios = leerJSON(FILES.usuarios);
  const limite = Date.now() - minutos * 60 * 1000;
  return usuarios.filter((u) => u.grupo === grupo && u.fecha_entrada >= limite);
}
