...
const LINK_WHATSAPP_REGEX = /chat\.whatsapp\.com\/[A-Za-z0-9]+/i;
...
/**
 * Revisa si el mensaje contiene un link de invitación a OTRO grupo de WhatsApp
 * (chat.whatsapp.com/...). A diferencia de otros links, este expulsa de inmediato
 * desde el primer mensaje -- no hay razón legítima común para compartir eso en
 * un grupo que no sea invitar gente a otro lado.
 */
async function revisarLinkWhatsApp(sock, grupoId, jid, msg, texto, avisarYExpulsar) {
  if (!LINK_WHATSAPP_REGEX.test(texto)) return false;

  try {
    await sock.sendMessage(grupoId, { delete: msg.key });
  } catch (e) {
    console.log('No se pudo borrar mensaje con link de WhatsApp:', e.message);
  }

  const nombre = jid.split('@')[0];
  const mensaje = `🚫 @${nombre}, los enlaces de invitación a otros grupos de WhatsApp no están permitidos. Serás expulsado.`;
...
async function revisarAntilink(sock, grupoId, jid, msg, texto, avisarYExpulsar) {
  // Primero, el caso más estricto: link de invitación a otro grupo de WhatsApp,
  // expulsión inmediata sin esperar repetición.
  const fueLinkWhatsApp = await revisarLinkWhatsApp(sock, grupoId, jid, msg, texto, avisarYExpulsar);
  if (fueLinkWhatsApp) return true;
