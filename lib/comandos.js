...
const { comandoProtegerAdmins } = require('./protegerAdmins');
...
case 'protegeradmins': {
  if (!esAdmin) return true;
  await comandoProtegerAdmins(sock, grupoId, sender, args[0]);
  return true;
}
    ...
    case 'menu': {
      await sock.sendMessage(grupoId, {
        text:
          ...
          '```!protegeradmins on/off```🛡️ Bloquea admins no autorizados\n' +
