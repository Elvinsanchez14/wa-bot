...
async function manejarCambioParticipantes(sock, update) {
  ...
  if (action === 'add') {
    for (const jid of participants) {
      ...
      const plantilla = db.getConfig(grupoId, 'bienvenida_texto');
      const texto = plantilla
        .replace(/\{nombre\}/g, `@${nombre}`) // incluye el @ aquí, así la plantilla no necesita escribirlo
        .replace(/\{minutos\}/g, toleranciaMin);
